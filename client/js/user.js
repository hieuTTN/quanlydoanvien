...
async function login() {
    ...
    if (response.status < 300) {
        ...
        if(checkQuanLy == true){
            listButton += `<a href="/doanvien/index.html" class="role-btn btn-doan-sinh">Quản lý</a>`;
        }

        if(checkQuanLy == true && !roleNames.includes("ROLE_ADMIN") && !roleNames.includes("ROLE_DOAN_SINH")){
            window.location.href = '/quanly/index.html';
        }
        else if(roleNames.length == 1){
            if(roleNames.includes("ROLE_DOAN_SINH")){
                window.location.href = '/doanvien/index.html';
            }
            if(roleNames.includes("ROLE_ADMIN")){
