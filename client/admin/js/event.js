        ...
        main += ` <tr> 
                    ...
                    <td>
                        <div class="d-flex gap-1">
                            ...
                            <button onclick="deleteEvent(${list[i].id})" class="btn btn-sm btn-danger"><i class="bi bi-trash"></i></button>
                        </div>
                        <div class="d-flex gap-1" style="margin-top: 5px;">
                            <button onclick="loadDetailEvent(${list[i].id})" data-bs-toggle="modal" data-bs-target="#eventDetailModal" class="btn btn-sm btn-info text-white"><i class="bi bi-eye"></i></button>
                            <button onclick="loadDetailEvent(${list[i].id})" data-bs-toggle="modal" data-bs-target="#eventDetailModal" class="btn btn-sm btn-primary text-white"><i class="bi bi-people"></i></button>
                        </div>
                    </td>
                </tr>`
    }
...
async function deleteEvent(id) {
    ...
    async function(isConfirm){
        if (isConfirm) {
            const response = await fetch(`http://localhost:8080/api/events/admin/delete?id=${id}`, {
                method: 'DELETE',
                headers: new Headers({
                    'Authorization': 'Bearer ' + token
                })
